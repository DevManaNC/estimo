...
export default function TravauxTab({ current, calc, setQty, setMatC, setMoC, addCustomItem, removeCustomItem, setNote }) {
  ...
  // Merge base items with custom items for current step
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 3, overflowX: "auto", padding: "0 0 12px" }}>
        {ETAPES.map((e, i) => (
          <button key={e.id} onClick={() => { setActiveStep(i); setSearch(""); }} style={{
            padding: "8px 10px", background: activeStep === i ? e.color + "20" : "#0a0a14",
            border: activeStep === i ? `1px solid ${e.color}55` : "1px solid #1a1a30",
      ...
      {/* Total TTC banner */}
      <div style={{ display: "flex", alignItems: "center", marginBottom: 12, padding: "8px 12px", background: "#0a0a14", borderRadius: 8, justifyContent: "space-between" }}>
        <input
      ...
      <div style={{ ...si.card, borderColor: etape.color + "33" }}>
        <div style={{ padding: "14px 16px", background: etape.color + "08", borderBottom: `1px solid ${etape.color}22` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <div>
              ...
              <span style={{ fontSize: 11, color: "#666", marginLeft: 10 }}>TVA {etape.tva}%</span>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 10, color: C.muted }}>TOTAL ÉTAPE TTC</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: detail.ttc > 0 ? etape.color : "#333" }}>{fmtDec(detail.ttc)}</div>
            </div>
          </div>
          {detail.ht > 0 && (
            <div style={{ display: "flex", gap: 16, marginTop: 8 }}>
              <span style={{ fontSize: 11, color: C.muted }}>Mat: <b style={{ color: "#aaa" }}>{fmt(detail.matT)}</b></span>
              <span style={{ fontSize: 11, color: C.muted }}>MO: <b style={{ color: "#aaa" }}>{fmt(detail.moT)}</b></span>
            </div>
          )}
        </div>
        ...
        {filteredItems.map(item => {
          ...
          return (
            <div key={item.id} style={{ borderBottom: "1px solid #0e0e1a" }}>
              <div style={{ display: "flex", gap: 6, padding: "8px 16px", alignItems: "center", background: q > 0 ? "#0a0f0a" : "transparent" }}>
                <div style={{ flex: "1 1 180px", minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: q > 0 ? "#ddd" : C.muted }}>{item.label}</span>
                ...
                <div style={{ width: 80, display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 4 }}>
                  ...
                  {isCustom && <button onClick={() => removeCustomItem(etape.id, item.id)} style={{ background: "none", border: "none", color: "#e74c3c", cursor: "pointer", fontSize: 11, padding: 0 }}>&#x2715;</button>}
                </div>
              </div>
              ...
              {q > 0 && (
                <div style={{ padding: "0 16px 6px" }}>
                  <input
                    style={{ ...si.input, fontSize: 11, padding: "4px 8px", background: "#08080f", color: C.muted }}
